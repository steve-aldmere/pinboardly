...
function normalizeHref(u: string) {
  const t = u.trim();
  if (!t) return "";
    ...
    <main className="p-6 max-w-3xl mx-auto">
        ...
        >
          ...
          <div>
            ...
            <input
              ...
              type="text"
              className="mt-1 w-full border rounded px-3 py-2 text-sm"
              placeholder="e.g. Booking form, Kit list..."
            />
          </div>
      ...
      {rows.length === 0 ? (
        ...
        <div className="mt-6 space-y-3">
          {rows.map((p) => {
            const href = p.url ? normalizeHref(p.url) : "";
            return (
              <div key={p.id} className="border rounded-lg p-4">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    {p.content ? (
                      <div className="font-medium break-words">{p.content}</div>
                    ) : null}
